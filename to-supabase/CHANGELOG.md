...
- The published peer range on `@noy-db/to-postgres` is now `^0.8.0`. At 0.8.0-pre.0 the manifest
  carried `workspace:*`, which publishes as an **exact** version, so an installer of
  `@noy-db/to-supabase@0.8.0-pre.0` was pinned to `@noy-db/to-postgres@0.8.0-pre.0`
...
### BREAKING: factory renamed to `toSupabase()` ([#18](https://github.com/vLannaAi/noy-db-to/pull/18))

- `supabase()` → `toSupabase()` — every extended store now exports a `to<Backend>()`-named factory matching the package-prefix grammar. Import sites change; options and behavior are identical.
