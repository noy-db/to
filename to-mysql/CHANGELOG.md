...
### BREAKING: factory renamed to `toMysql()` ([#18](https://github.com/vLannaAi/noy-db-to/pull/18))

- `mysql()` → `toMysql()` — every extended store now exports a `to<Backend>()`-named factory matching the package-prefix grammar. Import sites change; options and behavior are identical.
