...
- `to-ssh` descriptors can no longer set anything the store's `DescriptorOptions` / `Address` types declare no field for. The factory destructures named fields instead of spreading the descriptor's unchecked `options` (and, where applicable, `address`) bag into the store options. Where the winning key was applied *conditionally*, a matching key in that bag previously survived and won — here: `remotePath` (applied only when `address.path` was set). New `#69` tests assert an undeclared key cannot reach the store.
...
### BREAKING: factory renamed to `toSsh()` ([#18](https://github.com/vLannaAi/noy-db-to/pull/18))

- `ssh()` → `toSsh()` — every extended store now exports a `to<Backend>()`-named factory matching the package-prefix grammar. Import sites change; options and behavior are identical.
