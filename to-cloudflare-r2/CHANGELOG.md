...
### BREAKING: factory renamed to `toCloudflareR2()` ([#18](https://github.com/vLannaAi/noy-db-to/pull/18))

- `r2()` → `toCloudflareR2()` — every extended store now exports a `to<Backend>()`-named factory matching the package-prefix grammar. Import sites change; options and behavior are identical.
